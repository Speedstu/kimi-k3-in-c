#!/usr/bin/env python3
"""One-shot source transformer for the local K3 max-parity branch.

This intentionally refuses to edit when an expected upstream fragment is not unique.
The helper is deleted before the feature PR is merged.
"""
...
s = once(
    ...
    "CLI source exclusivity message",
)

# Validation is inserted immediately before the existing --threads check.
s = once(
    s,
...
# Validate stop id once the checkpoint vocabulary is known and initialise the sampler.
s = once(
    s,
...
# Print the effective decoding contract next to thread info.
s = once(
    s,
...
# Read arbitrarily long ID prompts from a file, not argv. The actual engine still caps
# them at K3_MAX_PROMPT / model context, so this only removes the OS argv ceiling.
old_ids_parse = '''    } else {
...
'''
s = once(s, old_ids_parse, new_ids_parse, "CLI ids-file reader")

# Only the three places that choose a newly emitted authoritative token switch from
# argmax to the requested sampler. Speculative verification remains greedy internally,
# and sampled mode refuses --spec above until rejection sampling is implemented.
s = once(
    s,
...
s = p.read_text()
# Remove an obsolete regex assignment immediately overwritten by the real one.
