...
static uint32_t rs=0x8a5cd789u;
...
static void init_scale(void){ for(int i=0;i<255;i++){ union{uint32_t u; float f;} v; /* exactly the float power-of-two used by engine */
    if(i==0) v.f=0x1p-127f; else v.u=(uint32_t)i<<23; /* exponent byte i, mantissa zero */
    e8half[i]=(double)v.f*0.5; } e8half[255]=0.0; }
...
#pragma omp parallel for schedule(static) if(rows>64)
    ...
    for(int r=0;r<rows;r++){
        ...
        for(int g=0;g<ngrp;g++){
            ...
            const unsigned char *pb=pr+(size_t)g*16; const float *xg=x+(size_t)g*32;
...
#define F4(V,I128,XOFF) do { (V)=_mm256_fmadd_pd(_mm256_cvtepi32_pd((I128)),_mm256_cvtps_pd(_mm_loadu_ps(xg+(XOFF))),(V)); } while(0)
...
int main(void){
    enum{IN=3584,ROWS=3072,REPS=10}; const int pcols=IN/2,ngrp=IN/32;
    init_scale();
    ...
    for(size_t i=0;i<(size_t)ROWS*ngrp;i++){sc[i]=(unsigned char)(120+rnd()%15);if((rnd()&4095u)==0)sc[i]=255;}
    for(int i=0;i<IN;i++)x[i]=((int)(rnd()&0xffffu)-32768)*(1.0f/65536.0f);
    k3_matmul_mxfp4(a,x,pk,sc,IN,ROWS,32); cand(b,x,pk,sc,IN,ROWS);
    printf("base=%016llx cand=%016llx\n",hashf(a,ROWS),hashf(b,ROWS));
    if(memcmp(a,b,(size_t)ROWS*sizeof(float))){int bad=0;for(int i=0;i<ROWS;i++)if(memcmp(a+i,b+i,4)){if(bad<8)printf("row %d %.9g %.9g\n",i,a[i],b[i]);bad++;}printf("PARITY FAIL %d\n",bad);return 1;}
    puts("PARITY PASS");
    double ta=0,tb=0; for(int r=0;r<REPS;r++){double t=now_s();k3_matmul_mxfp4(a,x,pk,sc,IN,ROWS,32);ta+=now_s()-t;t=now_s();cand(b,x,pk,sc,IN,ROWS);tb+=now_s()-t;}
    ta/=REPS;tb/=REPS;
#ifdef _OPENMP
    printf("threads=%d ",omp_get_max_threads());
#endif
    printf("base=%.3fms cand=%.3fms speedup=%.3fx\n",ta*1e3,tb*1e3,ta/tb);
    return 0;
}
