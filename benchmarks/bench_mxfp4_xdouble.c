...
static void init_scale(void)
{
    for (int i = 0; i < 255; i++) {
        ...
        if (i == 0) v.f = 0x1p-127f;
        else v.u = (uint32_t)i << 23;
        e8half[i] = (double)v.f * 0.5;
    }
    e8half[255] = 0.0;
}

static void cand(float *y, const float *x, const unsigned char *packed,
                 const unsigned char *scales, int in, int rows)
{
...
#pragma omp parallel for schedule(static) if(rows > 64)
    ...
    for (int r = 0; r < rows; r++) {
        ...
        for (int g = 0; g < ngrp; g++) {
            ...
            const double *xg = xd + (size_t)g * 32;
            ...
            const __m128i hi = _mm_shuffle_epi8(
                lut, _mm_and_si128(_mm_srli_epi16(b, 4), mask));
...
#define F4(V, I128, O) do {                                                     \
                (V) = _mm256_fmadd_pd(_mm256_cvtepi32_pd((I128)),              \
                                      _mm256_loadu_pd(xg + (O)), (V));          \
            } while (0)
            F4(v0, _mm256_castsi256_si128(i0), 0);
            F4(v1, _mm256_extracti128_si256(i0, 1), 4);
            F4(v0, _mm256_castsi256_si128(i1), 8);
            F4(v1, _mm256_extracti128_si256(i1, 1), 12);
            F4(v0, _mm256_castsi256_si128(i2), 16);
            F4(v1, _mm256_extracti128_si256(i2, 1), 20);
            F4(v0, _mm256_castsi256_si128(i3), 24);
            F4(v1, _mm256_extracti128_si256(i3, 1), 28);
#undef F4
            double a[4];
...
int main(void)
{
    enum { IN = 3584, ROWS = 3072, REPS = 10 };
    const int pcols = IN / 2, ngrp = IN / 32;
    init_scale();

    unsigned char *pk = malloc((size_t)ROWS * pcols);
    unsigned char *sc = malloc((size_t)ROWS * ngrp);
    float *x = malloc((size_t)IN * sizeof(float));
    float *a = malloc((size_t)ROWS * sizeof(float));
    float *b = malloc((size_t)ROWS * sizeof(float));
    ...
    for (size_t i = 0; i < (size_t)ROWS * ngrp; i++) {
        ...
        if ((rnd() & 4095u) == 0) sc[i] = 255;
    }
    for (int i = 0; i < IN; i++)
        x[i] = ((int)(rnd() & 0xffffu) - 32768) * (1.0f / 65536.0f);

    k3_matmul_mxfp4(a, x, pk, sc, IN, ROWS, 32);
    cand(b, x, pk, sc, IN, ROWS);
