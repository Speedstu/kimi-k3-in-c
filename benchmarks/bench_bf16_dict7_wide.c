...
#undef Q8
    ...
    for (; i < n; i++) {
        const size_t j = i >> 3;
        const unsigned b = (unsigned)(i & 7u);
        const unsigned q = ((p0[j] >> b) & 1u)
                         | (((p1[j] >> b) & 1u) << 1)
                         | (((p2[j] >> b) & 1u) << 2);
        dst[2 * i] = low[i];
...
int main(int argc, char **argv)
{
    ...
    unsigned char *p0 = calloc(pb, 1), *p1 = calloc(pb, 1), *p2 = calloc(pb, 1);
    unsigned char *lit = malloc(n / 8 + 32);
    ...
    if (used != ne || memcmp(raw, out, raw_bytes)) {
        fprintf(stderr, "ROUNDTRIP FAIL %zu/%zu\n", used, ne);
        return 1;
    }
    ...
    for (int r = 0; r < reps; r++) {
        ...
        used = decode_wide(out, n, low, p0, p1, p2, lit, d7);
