...
static double uniform01(K3Sampler *s)
{
...
int k3_sample_token(K3Sampler *s, const float *logits, int n)
{
    ...
    if (!(s->temperature > 0.0) || !isfinite(s->temperature)) return -1;
    if (!(s->top_p > 0.0 && s->top_p <= 1.0) || !isfinite(s->top_p)) return -1;
    ...
    /* Agentic K3 benchmark parity uses top_p=1.0. Keep that important path O(V), with
     * no 160k-element sort. */
    if (s->top_p >= 1.0) {
        double total = 0.0;
        for (int i = 0; i < n; i++) total += exp((double)logits[i] / s->temperature - max_scaled);
        if (!(total > 0.0) || !isfinite(total)) return -1;
        const double target = uniform01(s) * total;
        double acc = 0.0;
        for (int i = 0; i < n; i++) {
    ...
    const double target = uniform01(s) * kept;
    double acc = 0.0;
    int answer = c[nkeep - 1].id;
